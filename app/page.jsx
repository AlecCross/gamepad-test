...
// components/GamepadTester.js
import React, { useEffect, useRef, useState } from 'react';
import styles from './GamepadTester.module.css';

// *** 1. Імпортуйте сюди ваші SVG-компоненти ***
// Наприклад:
// import ControllerBody from '../../public/assets/svg/controller_body.svg';
// import ButtonA from '../../public/assets/svg/button_a.svg';
// import ButtonB from '../../public/assets/svg/button_b.svg';
// import ButtonX from '../../public/assets/svg/button_x.svg';
// import ButtonY from '../../public/assets/svg/button_y.svg';
// import DpadDown from '../../public/assets/svg/dpad_down.svg';
// import DpadUp from '../../public/assets/svg/dpad_up.svg';
// import DpadLeft from '../../public/assets/svg/dpad_left.svg';
// import DpadRight from '../../public/assets/svg/dpad_right.svg';
// import SelectButton from '../../public/assets/svg/select_button.svg';
// import LeftStickBase from '../../public/assets/svg/left_stick_base.svg';
// import LeftStickTop from '../../public/assets/svg/left_stick_top.svg';
// import RightStickBase from '../../public/assets/svg/right_stick_base.svg';
// import RightStickTop from '../../public/assets/svg/right_stick_top.svg';
// import LeftTrigger from '../../public/assets/svg/left_trigger.svg';
// import RightTrigger from '../../public/assets/svg/right_trigger.svg';
// import LeftBumper from '../../public/assets/svg/left_bumper.svg';
// import RightBumper from '../../public/assets/svg/right_bumper.svg';
// import HomeButton from '../../public/assets/svg/home_button.svg';
// import CenterCircle from '../../public/assets/svg/center_circle.svg';
// import CenterRect from '../../public/assets/svg/center_rect.svg';

const GamepadTester = () => {
  const [controllerIndex, setControllerIndex] = useState(null);
  const controllerAreaNotConnectedRef = useRef(null);
  const controllerAreaConnectedRef = useRef(null);
  const buttonsAreaRef = useRef(null);
  const rumbleOnButtonPressRef = useRef(null);
  const svgRef = useRef(null);
  const stickLXRef = useRef(null);
  const stickLYRef = useRef(null);
  const stickRXRef = useRef(null);
  const stickRYRef = useRef(null);
  const buttonRefs = useRef({});

  useEffect(() => {
    const handleConnectDisconnect = (event, connected) => {
      const gamepad = event.gamepad;
      console.log(gamepad);

      if (connected) {
        setControllerIndex(gamepad.index);
        if (controllerAreaNotConnectedRef.current) controllerAreaNotConnectedRef.current.style.display = "none";
        if (controllerAreaConnectedRef.current) controllerAreaConnectedRef.current.style.display = "block";
        createButtonLayout(gamepad.buttons);
        createAxesLayout(gamepad.axes);
      } else {
        setControllerIndex(null);
        if (controllerAreaNotConnectedRef.current) controllerAreaNotConnectedRef.current.style.display = "block";
        if (controllerAreaConnectedRef.current) controllerAreaConnectedRef.current.style.display = "none";
      }
    };

    const createAxesLayout = (axes) => {
      if (buttonsAreaRef.current) {
        buttonsAreaRef.current.innerHTML = '';
        for (let i = 0; i < axes.length; i++) {
          ...
          `;
          buttonsAreaRef.current.appendChild(axisDiv);
        }
      }
    };
    ...
    const createButtonLayout = (buttons) => {
      if (buttonsAreaRef.current) {
        ...
        for (let i = 0; i < buttons.length; i++) {
          buttonsAreaRef.current.innerHTML += createButtonHtml(i, 0);
        }
      }
    };
    ...
    const createButtonHtml = (index, value) => {
      return `<div class="button" id="button-${index}">
                ...
                <div className='button-text-area'>
                    <div className="button-name">B${index}</div>
                    <div className="button-value">${value.toFixed(2)}</div>
                </div>
            </div>`;
    };

    window.addEventListener("gamepadconnected", (event) => handleConnectDisconnect(event, true));
    window.addEventListener("gamepaddisconnected", (event) => handleConnectDisconnect(event, false));
    ...
    const updateButtonOnGrid = (index, value) => {
      ...
      if (buttonArea) {
        ...
        if (buttonValue) buttonValue.innerHTML = value.toFixed(2);
    ...
    const updateControllerButton = (index, value) => {
      ...
      console.log(`Оновлення кнопки з індексом: ${index}, знайдений елемент:`, buttonElement);
      const selectedButtonClass = "selected-button";
      if (buttonElement) {
        if (value > 0) {
          buttonElement.classList.add(selectedButtonClass);
          buttonElement.style.filter = `contrast(${value * 200}%)`;
        } else {
    ...
    const updateStick = (elementId, leftRightAxis, upDownAxis) => {
      ...
      if (stick) {
        const x = Number(stick.dataset.originalXPosition) || Number(stick.getAttribute('cx'));
        const y = Number(stick.dataset.originalYPosition) || Number(stick.getAttribute('cy'));
    ...
    const handleRumble = (gamepad) => {
      if (rumbleOnButtonPressRef.current && rumbleOnButtonPressRef.current.checked && gamepad && gamepad.vibrationActuator && gamepad.buttons.some((button) => button.value > 0)) {
    ...
    return () => {
      window.removeEventListener("gamepadconnected", (event) => handleConnectDisconnect(event, true));
      window.removeEventListener("gamepaddisconnected", (event) => handleConnectDisconnect(event, false));
    };
  }, [controllerIndex]);

  return (
    <div>
      <h1 className={styles.center}>Gamepad Tester</h1>
      <div id="controller-not-connected-area" className={`${styles.controllerStatus}`} ref={controllerAreaNotConnectedRef}>
        <div className={styles.loader}></div>
        <div>Controller not connected. Press any button to start.</div>
      </div>
      <div id="controller-connected-area" ref={controllerAreaConnectedRef}>
        <div id="controller-connected" className={`${styles.controllerStatus} ${styles.connected}`}>Connected</div>
        <div id="rumble-on-button-press-area" className={styles.rumbleArea} ref={rumbleOnButtonPressRef}>
          <input id="rumble-on-button-press" type="checkbox" />
          <label htmlFor="rumble-on-button-press">Rumble on button press</label>
        </div>
        <div id="buttons" ref={buttonsAreaRef}></div>
      </div>
      ...
      <div className={styles.center}>
        <a href="https://github.com/CodingWith-Adam/gamepad-tester">GitHub Repository</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="https://youtu.be/tQyrpcOK6U0">YouTube Tutorial</a>
      </div>
    </div>
  );
};
